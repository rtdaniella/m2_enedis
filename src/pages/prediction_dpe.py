...
df = pd.read_csv("C:/Users/danie/OneDrive/Documents/GitHub/m2_enedis/src/files/dpe-nettoye.csv")  # Remplace "ton_fichier.csv" par le chemin réel de ton fichier CSV

# Liste des variables à inclure dans le formulaire
form_fields = [
    'type_batiment', 'code_postal_ban', 'classe_altitude',
    'cout_total_5_usages', 'etiquette_dpe', 'qualite_isolation_menuiseries',
    'type_installation_chauffage', 'conso_5_usages_e_finale',
    'qualite_isolation_enveloppe', 'qualite_isolation_plancher_bas',
    'zone_climatique', 'n_dpe', 'geopoint', 'surface_habitable_logement',
    'logement', 'qualite_isolation_murs', 'n_etage_appartement',
    'type_energie_n_1', 'hauteur_sous_plafond', 'passoire_energetique',
    'periode_construction', 'timestamp_reception_dpe'
]
        ...
        if column in df.columns:
            ...
            # Sinon, on utilise un champ texte
                ...
                field = dbc.Row([
                    dbc.Label(column.replace("_", " ").capitalize(), width=4),
                    dbc.Col(
                        dbc.Input(type="text", id=f"{column}-input", placeholder=f"Entrez {column.replace('_', ' ')}"),
                        width=8
                    )
                ], className="mb-3")
...
def create_pred_dpe_page():
    return html.Div([
        html.H1("Prediction DPE", style={"text-align": "center", "margin-bottom": "20px"}),
        ...
        dbc.Row([
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody([
                        html.H4("Formulaire de prédiction", className="card-title"),
                        *form_inputs,  # Insertion des champs de formulaire générés dynamiquement
                        dbc.Button("Prédire", id="predict-button", color="primary", className="mt-3"),
                    ]),
                    className="shadow-sm",
                    style={"padding": "20px", "margin-right": "15px","backgroundColor":"lightgrey"}
                ),
                width=6
            ),
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody([
                        html.H4("Résultat de la prédiction", className="card-title"),
                        html.Div(id="prediction-result", className="mt-3"),  # Zone d'affichage du résultat
                    ]),
                    className="shadow-sm",
        ...
        ], justify="center", style={"margin-top": "20px", "margin-left": "260px"})  # Ajustement du positionnement
    ])
